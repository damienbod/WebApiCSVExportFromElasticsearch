...
namespace WebApiCSVExportFromElasticsearch.Controllers
{
    public class PersonsCsvExportController : ApiController
    {
		...
		{
			hubContext.Clients.All.addDiagnosisMessage(string.Format("Csv export starting"));
			...
			{
				context.TraceProvider = new SignalRTraceProvider(hubContext, TraceEventType.Information);

				hubContext.Clients.All.addDiagnosisMessage(string.Format("ScanAndScrollConfiguration: 1s, 300 items pro shard"));
				hubContext.Clients.All.addDiagnosisMessage(string.Format("sending scan and scroll _search"));
				hubContext.Clients.All.addDiagnosisMessage(BuildSearchMatchAll());
				...
				hubContext.Clients.All.addDiagnosisMessage(string.Format("Total Hits: {0}", scrollIdResult.TotalHits));
				...
				while (scrollIdResult.TotalHits > processedResults)
				{
					...
					hubContext.Clients.All.addDiagnosisMessage(string.Format("Total Hits: {0}, Processed: {1}", scrollIdResult.TotalHits, processedResults));
				}
			}

			hubContext.Clients.All.addDiagnosisMessage(string.Format("Elasticsearch proccessing finished, starting to serialize csv"));
			return Ok(result);
		}
